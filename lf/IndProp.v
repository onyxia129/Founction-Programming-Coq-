...
(** Putting all of this together, we can translate the definition of
    ...
    rule: *)
(*递归方式定义谓词 H是证据  这个东西是个construct 所以可以destruct或者induction *)
...
Proof.
  ...
  (*inversion H as [| n' H' T]. inversion H'.*)
  apply E'.
Qed.
...
*)
(*第一句： 任取n 满足n<=n
  第二句： 已知H：任取n 满足n<=m 可以推出 任取n 满足 n<= m+1
*)
...
(*然后证明的逻辑是反过来的 要证明 n<= m 就要证明 n <= m - 1
也就是第二句话反过来 H变成结论这样化简
最后能化简到第一句话 那么这个就对了
第一句话相当于自己定义的一个对的东西。
*)

Inductive les_strict : nat -> nat -> Prop :=
  | les_strict_n (n : nat)                : les_strict n (S n)
  | les_strict_S (n m : nat) (H : les_strict n m) : les_strict n (S m).

Notation "n < m" := (les_strict n m).

Example test_les_strict: les_strict 3 5.
Proof.
  apply les_strict_S. apply les_strict_n.
Qed.
...
Inductive Total : nat -> nat -> Prop :=
  | Total_n_m (n m : nat)                : Total n m.

Example test_total : Total 168 20.
Proof.
  apply Total_n_m.
Qed.
...
Example test_empty : ~(Empty 168 20).
Proof.
...
| no_nil : nostutter []
| no_1 (x : X): nostutter [x]
| no_s (x:X) ( h::t : list X ) : x<>h /\ nostutter (h::t) -> nostutter(x::h::t).
 (* FILL IN HERE *)
.
