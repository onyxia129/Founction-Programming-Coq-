...
(*n,m 是全称量词 forall n m : nat, n + m = m + n 看成一个公式 *)
...
Proof.
  ...
  - apply HB.
Qed.
(*A B 是命题 可以把它们当做一个类型 HA HB 是对A B的证明*)
...
Proof.
  ...
  - destruct m as [|m'] eqn : Hm.
    + reflexivity.
    + apply Plus.plus_is_O in H. inversion H. auto.
Qed.
...
(*这里的[HP _] 因为后面的_部分不是感兴趣的（证明用不到），所以不给他名字*)
...
Proof. 
  intros.
  split.
  - intros. destruct H. apply H. destruct H.
  - intros. left. apply H.
Qed.
...
Proof.
  intros X P H.
  unfold not. intros [x0 Hx0].  apply Hx0. apply H.
Qed.
(** [] *)
...
Proof.
   intros. split.
   - intros [x1 Hx1]. destruct Hx1.
    + left. exists x1. apply H.
    + right. exists x1. apply H.
   - intros. destruct H.
    + destruct H as [x1 Hx1]. exists x1. left. apply Hx1.
    + destruct H as [x1 Hx1]. exists x1. right. apply Hx1.
Qed.
...
(*这是个命题哦 看着是个递归的 但是吧判断 的时候还是在做递归 返回类型是TRUE和FALSE 判断x在不在A这个list里面
看函数的返回类型是prop 所以是个命题
*)
...
Proof.
  ...
  - induction l as [|h t IH].
    ...
    + simpl. intros [H1 | H2].
      * exists h. auto.
      * apply IH in H2. inversion H2; subst. exists x. destruct H. auto.
  - induction l as [|h t IH].
    + simpl. intros[a Ha]. destruct Ha. auto.
    + simpl. intros [a [H1 [H2 | H3]]].
      * left. rewrite H2. apply H1.
      * right. apply IH. exists a. auto.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l.
   ...
   + simpl. intros [H1 H2]. intros x0. intros[H3 | H4].
     * rewrite <- H3. apply H1.
     * auto.
Qed.
(** [] *)
...
Proof.
  ...
  intros. induction n as [|s Hs].
  - simpl. exists 0. reflexivity.
  - rewrite evenb_S.  destruct (evenb s) eqn:H.
    + simpl. inversion Hs; subst. exists x. reflexivity.
    + simpl. inversion Hs; subst. exists (S x). simpl. reflexivity.
Qed.
(** [] *)
...
Theorem excluded_middle_irrefutable: forall (P:Prop),
  ~ ~ (P \/ ~ P).
Proof.
  unfold not. intros P H. apply H. right. intros. auto.
Qed.
