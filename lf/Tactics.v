...
(*这个地方就是 我要证的东西本质上是传递性，就是形式复杂一点，那么我已经先把普通的传递性证明好了 trans_eq
    这个地方就能用apply A with B  A 是已经证号的一个可以用的定理 B就是这次证明里面要用的参数，改一改  
...
(*injection 就是把两边一样的构造去掉 可以理解成化简条件*)
...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros. injection H as H1 H2. rewrite H0 in H2. injection H2 as H11.
  rewrite H1.   symmetry. apply H11.
Qed.
...
(** If we use [discriminate] on this hypothesis, Coq confirms
    ...
    intros H. auto. discriminate H. (*前提显然不成立*)
Qed.
...
Proof.
  ...
  -simpl. destruct m as [|m'] eqn:E.
    +simpl. reflexivity.
    +simpl. intros Hm. discriminate Hm.
  -simpl. destruct m as [|m'] eqn:E.
    +simpl. intros Hm. discriminate Hm.
    +simpl. intros Ht. apply f_equal. apply Hn. apply Ht.
Qed.
(** [] *)
...
(*
double_plus: forall n : nat, double n = n + n
double_injective: forall n m : nat, double n = double m -> n = m
*)
...
Proof.
  ...
  -simpl. destruct n as [|n'] eqn:H.
    +reflexivity.
    +simpl. intros T. discriminate T.
  -simpl. intros n H. induction n as [|n' Hn]. 
    +simpl. discriminate H.
    +simpl. apply S_injective in H. apply IHt in H. apply H.
Qed.
