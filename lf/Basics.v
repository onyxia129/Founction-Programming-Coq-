...
Definition nandbB (b1:bool) (b2:bool) : bool :=
  match b1 , b2 with
    |true , true => false
    |true , false => true
    |false , true => true
...
Definition nandbs (b1:bool) (b2:bool) : bool :=
  !(b1 && b2).

Definition nandbt (b1:bool) (b2:bool) : bool :=
  negb(andb b1 b2).

Compute (nandbs true false).
...
Definition andb3B (b1:bool) (b2:bool) (b3:bool) : bool
  := andb (andb b1 b2) b3.
...
Inductive nat : Type :=
  | O
  | S (n : nat).
(*上面这个是归纳定义 递归的用法  这个就是一开始讲的那个自然数集合*)
...
Definition success (n : nat) : nat :=
  match n with
    | O => S O
    | S n' => S n 
  end.

Compute success (S (S O)).
...
(*判断是不是偶数，没办法枚举,definition就不能用，所以要用递归的方法
上面第三条的意思是s（s n）和n的结果是一样的
*)
...
Fixpoint lebStrict (n m : nat) : bool :=
  match n, m with
  |_, O => false
  |O, S m' => true
  |S n', S m' => lebStrict n' m'
  end.
...
Proof.
  ...
  - reflexivity.
  - reflexivity.   Qed.
(*destruct ：分情况 [|]分两个情况 [|n']表示分的两个情况里左边那个定义，右边那个定义成n'//相当于给个名字 
以防万一后面要用 在这个的情况上，是n=o 和 n=sn' 两种情况 这个时候左边和右边其实都不要有名字的，
所以可以写成intros n. destruct n*)
...
(*Fixpoint function (n:nat) (m:nat) : nat :=
  match m , n with
  | O , _ => n + 1
  | S_ , O => function m' 1
  | S m' , S n' => function (m-1) (function m (n-1))
  end.
*)
(* ################################################################# *)
(** * More Exercises *)
...
Proof.
  intros. destruct (f b0) eqn:E.
    - destruct b0 eqn:Eb.
      + rewrite E. reflexivity.
      + rewrite H in E. discriminate.
    - rewrite H. rewrite H in E. symmetry. apply E.
Qed.
...
Theorem andb_eq_orb :
  ...
  b = c.
Proof.
  intros. destruct b0 eqn:Eb. simpl in H.
    - symmetry. apply H.
    - simpl in H. apply H.
Qed.
(** [] *)
