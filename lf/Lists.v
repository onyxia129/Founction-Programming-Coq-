...
Fixpoint oddmembers (l:natlist) : natlist :=
   match l with
    | nil => nil
    | h::t => if (! evenb h) then h :: oddmembers t else oddmembers t
  end.

Fixpoint leaveodd (l:natlist) : natlist :=
  match l with
    | nil => nil
    | h::t => if !evenb h then h :: leaveodd t else leaveodd t
  end.

Compute leaveodd [1;2;3;4].
...
Compute countoddmembers [1;2;3;4].
...
(*判断A是不是B的子集*)
Fixpoint Subset (l1 l2 : natlist) : bool :=
  match l1 with
  | nil=> true
  | h :: t => if (member h l2) then Subset t l2 else false
  end.
(*这个问题是元素重复的不能做，比如下面那个式子应该来说是false，但是重复的没办法算，所以算成true了
一般集合是没有重复的，但是这里上定义的bag是类集合，是有重复的元素的*)

Compute Subset [1;2;2;3] [1;2;3;4].
...
Theorem nil_app_ex : forall l : natlist,
 l ++ [] = l.
...
Proof.
 intros. induction l as [|h t].
  - reflexivity.
  - simpl. rewrite <- IHt. rewrite <- eqb_refl. reflexivity.
Qed.
(** [] *)
...
Lemma rev_help : forall(l1 l2 : natlist),
  l1 = l2 -> rev l1 = rev l2.
Proof.
  intros. f_equal. auto.
Qed.
...
Fixpoint nth_error'' (l:natlist) (n:nat) : natoption :=
  match l with
  | nil => None
  | a :: l' => let v := nth_error'' l' (pred n) in
                if n =? O then Some a
               else v
  end.
...
(** How _many_ elements does the type [baz] have? (Explain in words,
    in a comment.) *)
(*3个？感觉是类似树的结构Baz1，Baz2两个子节点，Baz2还带一个bool值*)
(* FILL IN HERE *)
