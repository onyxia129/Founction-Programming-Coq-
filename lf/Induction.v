...
Proof.
  ...
  rewrite -> plus_comm.
(*这里因为+是左结合，所以他交换律做的是外面的（m+n）和（p+q） 而不是我们想要换的m和n*)
...
(*-start- myAdd*)
Theorem mult_assign_1 : forall n m:nat, 
   n + n * m = n * (1 + m). 
Proof.
...
Proof.
    ...
    induction n as [|n' Hn'].
     ...
     -simpl. rewrite -> Hn'. rewrite -> mult_assign_1. simpl. reflexivity.
Qed.



Search (forall n m, n +  n * m  = n * (1 + m)).
